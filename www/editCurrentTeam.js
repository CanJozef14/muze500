...
var fifth; // autorun
var sixth; // switch autonomous
var seventh; // scale autonomous
  ...
  if (u) {
    retriveData(u)
      ...
      saveData.addEventListener('click', function() {
        ...
        db.ref('users/' + u.uid + '/currentEvent/currentTeam/').once('value', function(snap) {
        var intake;
        var switchopt;
        var scaleopt;
        var climbopt;
        ...
        firebase.database().ref('users/' + u.uid + '/currentEvent/teams/' + snap.val().skey).set(updatedVersion)
        })
...
function retriveData(user) {
  db.ref('users/' + user.uid + '/currentEvent/currentTeam/').once('value', function(snap) {
    teamName.value = snap.val().name
    teamNo.value = snap.val().teamNo
    adcom.value = snap.val().additional
    console.log(snap.val().autorundb === 1);
    if (snap.val().autorundb === 1) {
    autorun.checked = true
    }
    if (snap.val().switchautondb === 1) {
    switchh.checked = true
    }
    if (snap.val().scaleautondb === 1) {
    scale.checked = true
    }
  })
...
function doStuff() {
  console.log("opt" + first);
  document.getElementById("opt" + first).checked = true
}
